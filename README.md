...
- **Base Image**: Fat container with all development tools (Python, Go, SSH, zsh, code-server, opencode, Jupyter, scrapling)
...
**Image**: `ghcr.io/xtin59s/base:latest`
...
```bash
docker pull ghcr.io/xtin59s/base:latest
```
...
```bash
docker build -t base:latest images/base/
```
